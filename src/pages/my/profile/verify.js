...
class Verify extends React.Component {
  ...
  onTextChanged() {
    ...
    const realname = this.realname && this.realname.value;
    const idcard = this.idcard && this.idcard.value.replace(/(^\s+)|(\s+$)/g, "");
  ...
  onSubmit() {
    ...
    if (password) {
      data.pwd = base64_encode(password);
    }
      ...
      <div>
        {user.have_pwd == 0 ? (
          <div>
            ...
            <span className="page-my-profile-verify-text-pwd">大小写字母/数字/符号任意两种组合,长度8～20位</span>
          ...
          <div style={{ width: "100%", height: "100%" }}>
            <div className="page-my-profile-verify-main">
              ...
              {//名字
                this.renderName()}
              {//身份证
                shouldShowIdCard ? this.renderIdCard() : null
              }
              {/* {//民族
                shouldShowNation ? this.renderNation() : null
              } */}
              {//地址
                shouldShowAddr ? this.renderAddr() : null
              }
              {//密码
                this.renderPassword()}
              {//自定义信息
