...
export const payResultAction = data => ({
  ...
  payload: fetch(`/fundraisingproject/pay/result/${data.sn}`, { method: "GET" ,data})
});
...
export const joinPayProject = data => dispatch => {
  ...
  fetch(`/fundraisingproject/join/${data.id}`, {
    method: "POST",
    data
  })
    .then(json => {
      ...
      if (!window.userAgent && json.data.wechatPayUrl) {
        ...
        location.replace(`/fundraisingproject/receipt/${json.data.sn}?firstopen=true`);
        return;
      }
...
export const joinProjectAction = data => ({
  ...
  payload: fetch(`/fundraisingproject/join/${data.id}`, { data })
});
export const joinProjectActionAnswer = data => ({
  type: "PROJECT_JOINPROJECT_DATA",
  payload: fetch(`/fundraisingproject/answer/${data.id}`, { data })
});
