...
import "./signUp.css";
import { requestProjectDetail } from "../detail/detail.store";
import { joinPayProject, joinProjectAction } from "../sign/sign.store";
...
class SignUpPage extends React.Component {
  constructor(props) {
    ...
    this.CustomChildren = ({ extra, onClick }) => (
      ...
      >
        <span className="page-project-signUp-verify-text page-project-signUp-verify-text-lineheight">
          {extra}
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (
      ...
      !Ndetail.failed &&
      Ndetail.data &&
      Ndetail.data.custom_payment_config
    ) {
      let data = [];
      let total = null;
    ...
    return (
      <div>
        <div className="page-signUp-danxuan">
          {Number(item.is_required) == 1 ? (
            <span className="page-project-signUp-verify-header-start page-project-signUp-verify-header-other-start">
              *
            </span>
          ) : null}
        ...
        <List renderHeader={() => item1.label}>
          {data.map(i => (
            <CheckboxItem
              ...
              onChange={() => this.handleOtherInfoMoreClick(item1.key, i.label)}
            >
              {i.label}
            </CheckboxItem>
          ))}
    ...
    return (
      <div>
        <div>
          ...
          <input
            id={`${key}`}
            className="page-my-profile-signUp-verify-double-text"
  ...
  renderOtherInfoManyInput(item) {
    ...
    return (
      <div>
        ...
        <textarea
          id={`${key}`}
          className="page-project-signUp-edit-text"
          maxLength="200"
  ...
  renderOtherPic(item) {
    ...
    return (
      <div className="page-project-signUp-other-title">
        {Number(item.is_required) === 1 ? (
          <span className="page-project-signUp-verify-header-start page-project-signUp-verify-header-other-pic-start">
            *
          </span>
        ) : null}
        <div className="page-project-signUp-verify-header-box-pic-fonts">
          {data.label}
        </div>
        <div className="page-project-signUp-photo-container">
          {this.state[key].map((item, keys) => (
            <div className="page-project-signUp-item-render-container">
              ...
              <div
                className="page-project-signUp-item-render-del"
          ...
          {this.state[key].length === 1 ? (
            ...
            <div
              className="page-project-signUp-item-upload-container"
              id={`${key}`}
  ...
  renderOtherInfo() {
    if (
      this.props.detail.data === null ||
      this.props.detail.data.custom_config === null
    ) {
      return null;
    }
    return (
      <div>
        {this.props.detail.data.custom_config &&
        this.props.detail.data.custom_config.length
          ? this.props.detail.data.custom_config.map((item, index) => {
              switch (
                ...
                //日期时间空间
                case 7:
                  return <div key={index}>{this.renderTime(item)}</div>;
                  break;
                default:
                  return;
              }
      ...
      <div className="page-project-signUp">
        ...
        <div className="page-project-signUp-bottom-btn">
          ...
          <div className="page-project-signUp-bottom-btn-contain">
            ...
            {this.state.data ? (
              <div className="total">
                合计：<span>¥{getnum(this.state.total || 0)}</span>
              </div>
            ) : null}

            <div className="btn" onClick={this.onSubmmit}>
              提交
            </div>
          </div>
        </div>
